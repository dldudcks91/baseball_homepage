...
def trade_day(request):
    ...
    TEST_MINUTES= 0
    current_time = datetime(2025, 1, 18, 14, 35, 3, tzinfo = timezone.utc)#datetime.now(tzinfo = timezone.utc) #- timedelta(minutes = TEST_MINUTES)
...
def trade_swing(request):
    ...
    TEST_HOURS= 0
    current_time = datetime(2025, 1, 23, 17, 23, 3, tzinfo = timezone.utc).replace(minute =0, second= 0)
    ...
    last_60_time = get_current_time(current_time, -((24 * 1) + TEST_HOURS)*60)
    last_week_time = get_current_time(current_time, -((24 * 7) + TEST_HOURS)*60)
    last_4_week_time = get_current_time(current_time, -((24 * 28) + TEST_HOURS)*60)
    
    #시점데이터
    last_1_data = MarketHour.objects.filter(log_dt = last_1_time)
    last_60_data = MarketHour.objects.filter(log_dt = last_60_time)
    last_week_data = MarketHour.objects.filter(log_dt = last_week_time)
    last_4_week_data = MarketHour.objects.filter(log_dt = last_4_week_time)

    #고점데이터
    high_low_data = MarketHour.objects.filter(log_dt__gte= last_4_week_data, volume__gt = 0, price__gt = 0).values('market').annotate(max_price = Max('high_price'), min_price = Min('low_price'))
    
    #거래량,거래대금
    last_1_sum_data = Market.objects.filter(log_dt__gte= last_1_time, volume__gt = 0).values('market').annotate(total_volume=Sum('volume'), total_amount = Sum('amount'), cnt = Count('volume'))
    last_60_sum_data = MarketHour.objects.filter(log_dt__gte= last_60_time, volume__gt = 0).values('market').annotate(total_volume=Sum('volume'), total_amount = Sum('amount'), cnt = Count('volume'))
    ...
    last_4_week_sum_data = MarketHour.objects.filter(log_dt__gte= last_4_week_time, volume__gt = 0).values('market').annotate(total_volume=Sum('volume'), total_amount = Sum('amount'), cnt = Count('volume'))
    market_info_list = MarketInfo.objects.all().order_by('symbol')
    market_supply_list = MarketSupply.objects.all().order_by('symbol')
    ...
    market_list = [
        {
            ...
            'price_1m': next((d.price for d in last_1_data if d.market == item.market), None),
            'price_60m': next((d.price for d in last_60_data if d.market == item.market), None),
            'price_week': next((d.price for d in last_week_data if d.market == item.market), None),
            'price_4_week': next((d.price for d in last_4_week_data if d.market == item.market), None),
            'price_high': next((d['max_price'] for d in today_high_low_data if d['market'] == item.market), None),
            'price_low': next((d['min_price'] for d in today_high_low_data if d['market'] == item.market), None),
            
            'amount_1m': next((d['total_amount'] for d in last_1_sum_data if d['market'] == item.market), None),
            'amount_60m': next((d['total_amount'] for d in last_60_sum_data if d['market'] == item.market), None),
            'amount_week': next((d['total_amount'] for d in last_week_sum_data if d['market'] == item.market), None),
            'amount_4_week': next((d['total_amount'] for d in last_4_week_sum_data if d['market'] == item.market), None),

            'count_1m': next((d['cnt'] for d in last_1_sum_data if d['market'] == item.market), None),
            'count_60m': next((d['cnt'] for d in last_60_sum_data if d['market'] == item.market), None),
            'count_week': next((d['cnt'] for d in last_week_sum_data if d['market'] == item.market), None),
            'count_4_Week': next((d['cnt'] for d in last_4_week_sum_data if d['market'] == item.market), None)
